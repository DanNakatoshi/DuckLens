"""Fetch historical data for AAPL and NVDA (individual stocks)."""
...
def fetch_stock_data(tickers: list[str], years: int = 5) -> None:
    """
    Fetch 5 years of OHLCV data for individual stocks.
    ...
    """
    print(f"\n{'='*70}")
    print(f"FETCHING STOCK DATA: {', '.join(tickers)}")
    print(f"{'='*70}\n")

    end_date = datetime(2025, 9, 30)  # Match our current data
...
def main():
    ...
    fetch_stock_data(tickers, years=5)
